...
@section('content-header')
    <h1>
        {{ trans('register::forms.title.create form') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.register.form.index') }}">{{ trans('register::forms.title.forms') }}</a></li>
        <li class="active">{{ trans('register::forms.title.create form') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.register.form.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('register::admin.forms.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
